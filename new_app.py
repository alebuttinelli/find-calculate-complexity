...
def corrected_type_token_ratio(doc):
    """
    Calcola la CTTR (types / √(2 * tokens)) per un documento spaCy,
    escludendo punteggiatura, spazi e stop word italiane.
    """
    if doc is None or not hasattr(doc, "__iter__"):
        return 0.0

    # Filtra solo parole significative (no punteggiatura, spazi, stop word)
    tokens = [token for token in doc if not token.is_punct and not token.is_space and not token.is_stop]

    if not tokens:
        return 0.0

    # Lemmatizza e metti in minuscolo per evitare duplicati non significativi
    types = set(token.lemma_.lower() for token in tokens)

    return round(len(types) / sqrt(2 * len(tokens)), 2)

def gulpease_index(doc):
    """Calcola l'indice Gulpease, considerando solo le frasi con più di 3 parole reali (non punteggiatura/spazi)."""
    if doc is None or not hasattr(doc, "sents"):
        return 0  # Input non valido

    # Filtra le frasi che hanno almeno 4 token validi (no punteggiatura o spazi)
    valid_sentences = [
        sent for sent in doc.sents
        if len([t for t in sent if not t.is_punct and not t.is_space]) > 3
    ]
    sentences_count = len(valid_sentences)

    if sentences_count == 0:
        return 0

    # Estrai token validi (no punteggiatura/spazi) da frasi valide
    valid_tokens = [
        token for sent in valid_sentences
        for token in sent if not token.is_punct and not token.is_space and hasattr(token, "text")
    ]

    words_count = len(valid_tokens)
    letters_count = sum(len(token.text) for token in valid_tokens)

    # Evita divisione per zero
    if words_count == 0:
        return 0

    gulpease = 89 - (100 * letters_count / words_count) / 10 + (300 * sentences_count / words_count)
    return round(gulpease, 2)

def avg_sentence_length(doc):
    # Filtra le frasi con più di 3 parole (escludendo punteggiatura e spazi)
    valid_sentences = [
        sent for sent in doc.sents
        if len([token for token in sent if not token.is_punct and not token.is_space]) > 3
    ]

    # Conta solo le parole (escludendo punteggiatura e spazi) nelle frasi valide
    words_count = sum(
        len([token for token in sent if not token.is_punct and not token.is_space])
        for sent in valid_sentences
    )

    # Se non ci sono frasi valide, evita la divisione per zero
...
def locate_complexity(text, _nlp_model, _matcher_instance):
    ...
    doc = _nlp_model(text)
    cttr_value = corrected_type_token_ratio(doc)
    gulpease_value = gulpease_index(doc)
    ...
    """
    return legend + result_html, cttr_value, gulpease_value, avg_sent_len
...
if st.button("Analizza Testo"):
    if testo_utente:
        ...
        with col1:
          st.metric(label="Corrected Type-Token Ratio (CTTR)", value=cttr)
        with col2:
